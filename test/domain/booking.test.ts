...
describe("overlaps — conflito de horário (regra central do negócio)", () => {
	...
	it("rejeita intervalo invertido (defesa de contrato)", () => {
		expect(() => overlaps(at(11), at(10), at(10), at(11))).toThrow(
			/invariant|start >= end/i,
		);
		expect(() => overlaps(at(10), at(11), at(12), at(11))).toThrow(
			/invariant|start >= end/i,
		);
	});

	it("rejeita zero-duration (defesa de contrato)", () => {
		expect(() => overlaps(at(10), at(10), at(10), at(11))).toThrow(
			/invariant|start >= end/i,
		);
	});
});
