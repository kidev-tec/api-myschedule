...
function initFirebaseAdmin(projectId: string, serviceAccountB64?: string): void {
...
export function firebaseAuthMiddleware(
  ...
  return async (c, next) => {
    ...
    if (!header?.startsWith("Bearer ")) {
      return c.json({ error: "token ausente", hint: "Envie Authorization: Bearer <idToken do Firebase>" }, 401);
    }
    ...
    try {
      ...
      return c.json({ error: "token inválido ou expirado", hint: "Refaça o login no app" }, 401);
    }
  };
}
